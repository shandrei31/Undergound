...
export default function Checkout() {
  ...
  if (cart.length === 0)
    return <div className="p-20 text-center font-black uppercase">Cart is Empty</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 py-12 relative">
      ...
      {toast.message && (
        <div
          className={`fixed bottom-6 right-6 px-6 py-3 rounded shadow-lg text-white font-bold uppercase z-50 ${
      ...
      <h1 className="text-6xl font-black italic uppercase tracking-tighter mb-10 border-b-8 border-black pb-4 text-center">
        Checkout
      </h1>

      <div className="border-4 border-black p-8 bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] mb-10">
        <p className="mb-4 font-bold uppercase text-xs text-gray-400">
          Customer: {userEmail}
        </p>
        ...
        <div className="flex justify-between items-end border-t-4 border-black pt-4">
          ...
          <p className="font-black text-5xl tracking-tighter italic">
      ...
      <button
        onClick={placeOrder}
        disabled={loading}
        className="w-full bg-black text-white py-6 font-black uppercase text-2xl border-4 border-black hover:bg-white hover:text-black transition-all active:translate-y-1 disabled:opacity-50"
      >
