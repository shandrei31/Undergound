...
export default function Admin() {
  ...
  try {
    ...
    const productData = {
      ...formData,
      sizes: formData.category === "Streetwear" ? ['S','M','L','XL'] : null
    };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 min-h-screen bg-white relative">
      ...
      {toast.message && (
        <div className={`fixed bottom-6 right-6 px-6 py-3 rounded shadow-lg text-white font-bold uppercase z-50 transition-all ${toast.type === "success" ? "bg-green-600" : "bg-red-600"}`}>
          {toast.message}
        </div>
      )}
      ...
      <div className="flex gap-4 mb-10 border-b-4 border-black pb-4">
        <button onClick={() => setActiveTab("products")}
          className={`px-8 py-3 font-black uppercase italic border-4 border-black transition-all ${activeTab === 'products' ? 'bg-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)]' : 'bg-white text-black hover:bg-gray-100'}`}>
          Inventory
        </button>
        <button onClick={() => setActiveTab("orders")}
          className={`px-8 py-3 font-black uppercase italic border-4 border-black transition-all ${activeTab === 'orders' ? 'bg-black text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)]' : 'bg-white text-black hover:bg-gray-100'}`}>
      ...
      {activeTab === "products" ? (
        <div className="grid md:grid-cols-3 gap-10">
          ...
          <form onSubmit={handleProductSubmit} className="border-4 border-black p-8 space-y-5 h-fit sticky top-24 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-4 border-b-2 border-black pb-2">
              {editingProduct ? 'Update Item' : 'New Drop'}
            </h2>
            ...
            <div className="space-y-2">
              ...
              <input type="number" min={0} value={formData.stock} onChange={e => setFormData({...formData, stock:Number(e.target.value)})} className="w-full p-3 border-2 border-black font-bold outline-none bg-gray-50 focus:bg-white" required />
            </div>
            ...
            <div className="space-y-2">
              ...
              <select value={formData.category} onChange={e => setFormData({...formData, category:e.target.value})} className="w-full p-3 border-2 border-black font-bold outline-none bg-gray-50 focus:bg-white">
            ...
            {editingProduct && (
              <button type="button" onClick={() => {setEditingProduct(null); setFormData({name:"", description:"", price:"", category:"Streetwear", image_url:"", stock:0})}} className="w-full text-xs font-black uppercase underline mt-2 text-red-600 text-center">
                Cancel Edit
              </button>
            )}
          </form>
          ...
          <div className="md:col-span-2 flex flex-col gap-2">
            <div className="flex flex-col md:flex-row gap-4 mb-4 items-center">
              <input
                ...
                className="p-3 border-2 border-black font-bold outline-none w-full md:w-1/2 bg-gray-50 focus:bg-white"
              />
              <select
                value={categoryFilter}
                onChange={e => setCategoryFilter(e.target.value)}
                className="p-3 border-2 border-black font-bold outline-none w-full md:w-1/4 bg-gray-50 focus:bg-white"
              >
                ...
                <tbody>
                  {filteredProducts.map(p => (
                    <tr key={p.id} className="border-b-4 border-black hover:bg-gray-50 font-bold uppercase">
                      ...
                      <td className="p-4">
                        <input type="number" value={p.stock} min={0} onChange={async (e)=>{
                          const newStock = Number(e.target.value);
                          setProducts(products.map(prod => prod.id === p.id ? {...prod, stock:newStock} : prod));
                          try { await supabase.from('products').update({stock:newStock}).eq('id', p.id); showToast(`Stock updated for ${p.name}`,'success'); } catch(err){console.error(err); showToast('Failed to update stock!','error'); }
          ...
          <table className="w-full border-4 border-black">
            ...
            <tbody>
              {orders.map(order => {
                ...
                return (
                  <tr key={order.id} className={`border-b-4 border-black transition-all ${isCancelled ? 'bg-gray-100 opacity-50' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="p-4 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <select value={order.status} disabled={isCancelled} onChange={(e)=>updateOrderStatus(order.id,e.target.value)} className={`p-2 border-2 border-black font-black uppercase text-xs outline-none ${isCancelled ? 'bg-gray-400 text-white cursor-not-allowed' : order.status==='Shipped'?'bg-blue-500 text-white':order.status==='Delivered'?'bg-green-600 text-white':'bg-yellow-400'}`}>
                        ...
                        {isCancelled && (
                          <button onClick={() => handleDeleteOrder(order.id)} className="bg-red-600 text-white px-4 py-1 text-[10px] font-black border-2 border-black hover:bg-white hover:text-red-600 transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                            DELETE ORDER
                          </button>
                        )}
                      </div>
